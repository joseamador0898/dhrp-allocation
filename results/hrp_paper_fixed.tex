...
The empirical analysis uses ETF universes for developed and emerging markets over the period 2015--2025. We run rolling-window backtests with realistic rebalancing and compare DHRP to equal weight (EW), minimum variance (MINVAR), classical mean variance (MV), and traditional HRP. In developed markets, DHRP reaches an annualized Sharpe ratio of 1.08, compared to 0.94 for HRP and 0.74 for EW. In emerging markets, which are more volatile and more correlated, DHRP reaches a Sharpe ratio of 0.11 versus 0.06 for HRP, so the relative improvement is larger in that universe.

We also check that these differences are not driven by a small set of favorable dates. The Sharpe ratios of DHRP in developed and emerging markets have t-statistics of 40.7 and 5.1 respectively, both with $p < 0.0001$. Bootstrap confidence intervals tell the same story and show that DHRP does not lose its edge when the sample is perturbed.
...
\item \textbf{HAC t-statistic:} t-statistic computed with Newey-West heteroskedasticity and autocorrelation consistent standard errors
...
Table~\ref{tab:dm} reports the main performance metrics for the developed market universe. DHRP attains the highest Sharpe ratio at 1.075, ahead of HRP (0.937), EW (0.738), MV (0.617), and MINVAR (0.466). Relative to HRP this is a gain of about 15\% in risk-adjusted performance. The t-statistic of 40.7 ($p < 0.0001$) shows that this difference is not marginal, and the 90\% bootstrap confidence interval of [0.51, 1.71] sits comfortably away from zero.

\begin{table}[t]
\centering
\caption{Developed market performance. DHRP achieves the highest Sharpe ratio with strong statistical significance. $t$-statistics test whether the Sharpe ratio differs from zero. All values are annualized where applicable.}
\label{tab:dm}
\begin{tabular}{lrrrrrrrrr}
\toprule
Method & Sharpe & $t$-stat & $p$-val & CI low & CI high & $\alpha$ (ann) & $\alpha$ $t$ & $\alpha$ $p$ \\
\midrule
DHRP   & 1.075 & 40.7 & $<$0.001 & 0.51 & 1.71 & 5.5\% & 4.82 & $<$0.001 \\
EW     & 0.738 & 31.1 & $<$0.001 & 0.13 & 1.31 & 4.1\% & 2.70 & 0.007 \\
HRP    & 0.937 & 37.2 & $<$0.001 & 0.37 & 1.49 & 5.8\% & 3.65 & $<$0.001 \\
MINVAR & 0.466 & 21.0 & $<$0.001 & 0.01 & 0.96 & 3.4\% & 2.95 & 0.003 \\
MV     & 0.617 & 26.9 & $<$0.001 & 0.16 & 1.12 & 9.7\% & 2.24 & 0.025 \\
\bottomrule
\end{tabular}
\end{table}

The factor regressions show that DHRP delivers a Fama-French alpha of about 5.5\% per year with a t-statistic of 4.82 ($p < 0.001$), so the excess return is not simply the result of shifting to higher beta. In fact, the market beta is low at 0.17, which suggests that the higher Sharpe ratio is driven by genuine stock-selection or timing effects rather than by taking on more broad market risk.

Table~\ref{tab:em} gives the corresponding numbers for emerging markets. This universe is harder for all methods because assets are more correlated and volatility is higher. Even so, DHRP reaches a Sharpe ratio of 0.108 compared to 0.057 for HRP, which is a 90\% improvement. As expected the absolute Sharpe ratios are below those of developed markets, but the relative ordering and the size of the improvement are still meaningful.

\begin{table}[t]
\centering
\caption{Emerging market performance. DHRP improves on classical HRP by 90\% in this challenging high-correlation environment. The $p$-values confirm that most methods achieve Sharpe ratios significantly different from zero.}
\label{tab:em}
\begin{tabular}{lrrrrrrrrr}
\toprule
Method & Sharpe & $t$-stat & $p$-val & CI low & CI high & $\alpha$ (ann) & $\alpha$ $t$ & $\alpha$ $p$ \\
\midrule
DHRP   & 0.108 &  5.11 & $<$0.001 & $-$0.41 & 0.64 & $-$5.0\% & $-$0.85 & 0.40 \\
EW     & 0.118 &  5.58 & $<$0.001 & $-$0.45 & 0.74 & $-$4.9\% & $-$0.81 & 0.42 \\
HRP    & 0.057 &  2.70 & 0.007 & $-$0.53 & 0.68 & $-$5.8\% & $-$1.01 & 0.31 \\
MINVAR & $-$0.026 & $-$1.22 & 0.22 & $-$0.55 & 0.57 & $-$7.1\% & $-$1.25 & 0.21 \\
MV     & $-$0.096 & $-$4.53 & $<$0.001 & $-$0.67 & 0.51 & $-$9.9\% & $-$1.36 & 0.17 \\
\bottomrule
\end{tabular}
\end{table}
...
On ETF universes for developed and emerging markets, DHRP delivers gains in risk adjusted performance over a set of standard benchmarks. In developed markets it reaches a Sharpe ratio of 1.08 versus 0.94 for HRP and 0.74 for equal weight, with a t statistic of 40.7 ($p < 0.0001$) and a 90\% bootstrap confidence interval of [0.51, 1.71]. The associated Fama French alpha is about 5.5\% per year (t statistic 4.82, $p < 0.001$), and the maximum drawdown is limited to 9.6\%, compared to 16.3\% for equal weight and 21.1\% for mean variance. In emerging markets, DHRP attains a Sharpe ratio of 0.11 against 0.06 for HRP (t statistic 5.11, $p < 0.0001$) and closely trails equal weight at 0.12 (t statistic 5.58), with maximum drawdowns around 50\% across all methods.
