...
\citep{FamaFrench2015,Carhart1997} factor alphas under multi-seed ($n=10$) protocols
...
We report eight headline methods--DHRP, an LLM-augmented variant
(LLM-DHRP), and six classical baselines (EW, MV, MINVAR, HRP, RP,
MAXDIV)--with four additional deep allocators implemented for
supplementary comparisons.
%
DHRP achieves out-of-sample Sharpe ratios of
$\mathbf{1.304 \pm 0.10}$ on commodities ($\text{PSR}=0.999$) and
$\mathbf{1.290 \pm 0.05}$ on crypto ($\text{PSR}=0.991$).
DHRP beats classical HRP in six of eight universes (the two losses,
sectors and factors, are within 0.03 Sharpe of HRP) and is the best
risk-parity-family method in DM ($0.446 \pm 0.06$ vs.\ HRP $0.216$,
EW $0.255$). Fama--French 3-factor regression on commodities gives
$\alpha_{ann}=6.3\%$ ($t=1.58$); the headline therefore reflects a
positive sample Sharpe (PSR$=0.999$) rather than a significant
risk-adjusted alpha. Multi-method tests (MCS at $\alpha=0.05$) cannot
distinguish DHRP from the best classical baseline in commodities or
crypto.
%
...
A row-wise entropy-regularized interpretation of the soft assignment
matrix (Appendix~\ref{app:ot}) connects the assignment to the Sinkhorn
literature, although our implementation enforces only row-stochasticity
rather than the full bi-stochastic Sinkhorn iteration.
...
learning rate (Appendix~\ref{app:hyperparams}).
...
Section~\ref{sec:llmablation} should therefore be read as a
...
does not improve the headline Sharpe (Section~\ref{sec:llmablation}).
...
% NeurIPS 2026 mandatory paper checklist (does not count toward 9-page main limit).
...
\textbf{Justification:} The abstract and Section~\ref{sec:intro} claim three contributions: (i) DHRP architecture as an HRP-inspired differentiable relaxation, (ii) the 8-universe DHRP-8 benchmark with multi-seed PSR reporting, and (iii) honest negative LLM ablation. Each contribution is delivered in Sections~\ref{sec:method}, \ref{sec:benchmark}, and \ref{sec:llmablation} respectively.
...
\textbf{Justification:} The paper does not rely on an exact-theorem claim for the implemented layer. Section~\ref{sec:dhrp_layer} states the architecture scope and explicitly says DHRP is an HRP-inspired differentiable relaxation rather than a differentiable implementation of the full classical algorithm.
...
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} All neural methods report mean$\pm$std over 10 random seeds. Significance is reported through (i) Probabilistic Sharpe Ratios \citep{Bailey2012PSR}, (ii) HAC Newey--West t-statistics \citep{NeweyWest1987}, (iii) Fama--French factor regression alphas, (iv) pairwise Sharpe-difference tests via stationary block bootstrap \citep{PolitisRomano1994} with Holm--Bonferroni correction, and (v) SPA \citep{Hansen2005} and MCS \citep{HansenLundeNason2011} multi-method tests.
...
\textbf{Answer:} \answerYes{}\\
\textbf{Justification:} The work uses public market data, public news headlines fetched by scripts subject to upstream provider terms, and a publicly available pre-trained LLM (FinBERT, ProsusAI). No human subjects, no PII. The Croissant metadata documents survivorship bias and other dataset caveats per the NeurIPS RAI guidelines.
...
The implemented DHRP layer is intentionally presented as an
HRP-inspired relaxation. Classical HRP recursively splits budget across
both children of each tree node. The DHRP layer instead computes
temperature-scaled routing probabilities over a learned soft-gating
tree, combines them with learned asset-to-leaf assignments, and blends
the result with inverse volatility. This produces a differentiable
allocator that can be trained end-to-end and regularized toward HRP
weights, but it is not a differentiable clone of agglomerative
clustering plus recursive bisection.

This distinction matters in the hard-routing limit. As temperatures
shrink, the gate probabilities concentrate, while classical HRP still
allocates nonzero capital recursively across many subtrees. Exact
classical-HRP reproduction would require a different layer in which
each internal node emits explicit split-budget proportions for both
children, with the tree fixed to the HRP quasi-diagonal ordering.

\section{Row-wise Entropy-Regularized View of the Soft Leaf Assignment}
\label{app:ot}

The DHRP soft leaf-assignment matrix admits an interpretation as a
row-wise entropy-regularized assignment, in the same family as
Sinkhorn-style optimal transport \citep{Cuturi2013Sinkhorn} but with
only the row marginals enforced.

\begin{proposition}[Row-wise OT relaxation]
\label{prop:ot}
...
asset marginal $\mu_i = 1/n$. Then $\bar A$ is the unique solution to
the entropy-regularized assignment problem
\[
\min_{\Pi}\ \langle \Pi, C \rangle + \sum_{i,\ell} \Pi_{i,\ell}
\log \Pi_{i,\ell}, \quad \text{subject to } \sum_\ell \Pi_{i,\ell}
= \mu_i\ \forall i,
\]
i.e.\ the row-stochastic relaxation of entropy-regularized optimal
transport. The column marginal $\bar A^\top \mu$ is left
unconstrained; this is \emph{not} a full Sinkhorn solution, which
would additionally enforce a fixed column marginal $\nu$ via iterative
bi-stochastic normalization. Calling the construction ``Sinkhorn
equivalence'' would overstate the result; we describe it as a
row-stochastic relaxation throughout.
\end{proposition}
...
large $|A|$ approaches a hard leaf assignment.
...
\begin{itemize}\itemsep0.05em
  \item \textbf{Distribution}: code, metadata, the canonical notebook,
    and the reproduction script. Raw upstream market/news archives are
    fetched dynamically by script and are not redistributed.
  \item \textbf{Schema}: logical record sets for universes, prices, and
    headline-derived FinBERT features generated during reproduction.
  ...
  \item \textbf{Responsible-AI}: Survivorship bias warning (static ETF
    ...
    inception); license boundaries: code/metadata are released by the
    authors, while fetched prices, news, and factor data remain subject
    to upstream provider terms.
...
\paragraph{Compute budget.} Full end-to-end reproduction takes about
7 GPU-hours on NVIDIA T4 or about 2 GPU-hours on A100, plus roughly
30 minutes of CPU time for figures, tables, and metadata validation.
The smoke-only path runs a CPU mini-panel integration test.
