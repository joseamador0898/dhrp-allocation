...
\begin{proposition}[HRP recovery, conditional form]
\label{prop:recovery}
Fix a covariance matrix $\Sigma \in \mathbb{R}^{n\times n}$ for which
classical HRP produces a unique single-linkage agglomerative tree
$T^\star$ with $n$ leaves and a quasi-diagonal asset ordering, and let
$w^{\text{HRP}}(\Sigma) \in \Delta^{n-1}$ be the resulting allocation.
Suppose the DHRP layer is configured so that (i) the tree depth
satisfies $2^L \geq n$, (ii) the asset-leaf assignment $A$ is one-hot,
matching the HRP quasi-diagonal ordering, (iii) at every internal node
the gate logits are aligned with the corresponding HRP split direction,
(iv) the leaf prior $\beta$ matches the cumulative inverse-volatility
budget that HRP's recursive bisection assigns to each leaf of
$T^\star$, and (v) the inverse-volatility blend is disabled with
$\alpha = 1$. Then in the limit $\tau_k \to 0$ for all internal nodes,
the DHRP forward pass on $\Sigma$ converges to $w^{\text{HRP}}(\Sigma)$.
\end{proposition}

\noindent\textit{Proof sketch.} Under the listed assumptions, hard-
route gates concentrate on the HRP cluster split direction at every
node, the soft assignment $A$ with one-hot rows reproduces leaf
membership, the leaf prior $\beta$ exactly matches the bisection
budgets, and $\alpha = 1$ disables the inverse-vol blend. Full
construction in Appendix~\ref{app:proof}.

This is a relaxation, not a universal-recovery result. The DHRP layer
does not differentiate through agglomerative clustering itself: it
learns an amortized mapping $(x_t, \Sigma_t) \mapsto w_t$ that admits
the HRP allocation as a fixed point at the limit above. We treat
``DHRP recovers classical HRP as $\tau \to 0$'' as informal shorthand
for this conditional statement throughout the paper.
...
headline with FinBERT \citep{Huang2023} ($d_T = 768$), L2-normalize per
asset, and aggregate to a per-asset vector via concatenated mean and
max-pooling, producing $\bar T_t \in \mathbb{R}^{n \times 2 d_T}$. We
then collapse the asset axis with a normalized mean to obtain a single
timestep-level text summary $u_t \in \mathbb{R}^{2 d_T}$. A
TextProjection MLP maps $u_t$ to $\mathbb{R}^f$, and a gated residual
...
DHRP is a differentiable, HRP-inspired allocator that reduces to a
classical HRP allocation under explicit limit assumptions
(Proposition~\ref{prop:recovery}). On DHRP-8, it achieves a Sharpe of
