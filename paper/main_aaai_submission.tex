...
% Double-blind submission build: [submission] hides the author block.
% For a named camera-ready / arXiv build, swap the two lines below.
...
Six families: PSR \citep{Bailey2012PSR}; HAC t-statistics
...
\citep{DieboldMariano1995}; the SPA \citep{Hansen2005} and MCS
\citep{HansenLundeNason2011} multi-method tests at $\alpha = 0.05$; and,
to guard against selection under multiple testing, the Deflated Sharpe
Ratio \citep{Bailey2014DSR} and the Probability of Backtest Overfitting
(PBO) estimated by combinatorially symmetric cross-validation
\citep{Bailey2017PBO}.
...
To control for selection across the method panel, we report the Deflated
Sharpe Ratio (DSR) and the Probability of Backtest Overfitting (PBO). On
commodities, DHRP retains a DSR of $0.978$ after deflating for the seven
allocators in the panel, so the headline Sharpe stays significant under
multiple-testing correction; the panel PBO is $0.012$ over $12{,}870$ CSCV
splits, meaning the in-sample-best allocator drops below the
out-of-sample median in only $1.2\%$ of splits. PBO is likewise low in
emerging markets ($0.001$) and moderate in developed markets ($0.256$),
the universe where DHRP's margin is thinnest; over the full eleven-method
developed-markets panel PBO is $0.296$. These figures indicate that the
commodity result is not an artifact of backtest overfitting. Per-universe
DSR values are reported in the supplementary material.
...
the PSR, the Deflated Sharpe Ratio, and the PBO guard against
sample-specific luck and selection under multiple testing
\citep{Bailey2012PSR,Bailey2014DSR,Bailey2017PBO}, a longer horizon would
be more convincing. The commodity headline rests on sample Sharpe rather than a
