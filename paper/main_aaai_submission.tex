...
% TODO(after rerun): paste DSR per method (results/{universe}_stats.csv,
% column DSR) and PBO per universe (results/pbo_summary.csv) into the
% bracketed slots below, then delete this comment.
...
commodities, DHRP retains a DSR of $[\text{DSR}_{\text{cmd}}]$ after
deflating for the configurations searched, and the panel PBO is
$[\text{PBO}_{\text{cmd}}]$ ($[\,n\,]$ CSCV splits), indicating that
selecting the best backtested allocator is $[\text{not~}]$driven by
overfitting. Per-universe DSR and PBO values are reported in the
